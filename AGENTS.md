...
- Day 2: root `.mcp.json` is shared MCP config; `.codex/config.toml` is generated by `tools/mcp/day2/host-config.mjs`. Four separate local stdio backends read filesystem, InsightHub Docker services, a dedicated Kubernetes lab namespace and Prometheus. Optional `docker-compose.day2.yml` adds only lab Prometheus.
- Day 3 local profile: Terraform owns namespace + tokenless ServiceAccount; Helm owns web/api/worker and explicitly ephemeral pgvector/Redis dependencies. This profile never counts as AWS/EKS/RDS/ElastiCache/IRSA/OIDC verification.
- Day 4 observability: kube-prometheus-stack in namespace `monitoring` on the same local kind cluster scrapes all five components. API and worker expose `/metrics` directly; PostgreSQL and Redis go through exporters; web is probed by blackbox because Next.js serves no metrics. Grafana, Prometheus and Alertmanager are local pods, never Grafana Cloud or an AWS managed service.
...
- Worker-only metrics live in `app.core.worker_metrics`, never in `app.core.metrics`: the API imports the latter and would export worker counters as constant zero, shadowing the real series. Metric labels stay bounded; no document ID, filename or user identifier becomes a label.
- Recording rules are named `insighthub:<signal>:<window>`; anomaly bands are mean + 3*stddev over a one hour window offset by ten minutes so a firing incident cannot inflate its own band. Aggregations that can be empty use `or vector(0)`.
...
- Day 2: `npm --prefix tools/mcp/day2 test`; `node tools/mcp/day2/probe.mjs`; `python3 tools/mcp/day2/host_probe.py`. Setup/debug/Inspector instructions: `tools/mcp/day2/README.md`. Host probe uses the Codex MCP API without starting model turns or additional agents.
- Day 3 local: `make day3-static`; `make day3-local-plan`; `make day3-local-up`; `make day3-local-smoke`. Destroy only the selected namespace with `CONFIRM_LOCAL_DESTROY=insighthub-dev make day3-local-down`.
- Day 4: `make day4-monitoring-up`; `make day4-app-up`; `make day4-rules`; `make day4-dashboard`; `make day4-loadgen-up`; `make day4-forward`; `make day4-status`. Incidents: `make day4-incident-1|2|3`, then `make day4-samples` and `make day4-verify`. Slack webhook goes in through `SLACK_WEBHOOK_URL=... make day4-slack`. Teardown of monitoring only: `make day4-down`.
...
- Day 2 scope: project MCP configs, `tools/mcp/day2`, optional Day 2 Compose overlay, `debug-session-day2.md`, `ai-prompts/day2.md`, and Day 2 evidence. Keep tokens/admin kubeconfig in ignored `tmp/day2` (0600); refresh the 8-hour token before calls. Enforce namespace RBAC, Secret denial, filesystem containment and server read-only controls; no production/AWS credentials or raw Docker inspection/log output.
- Day 3 local secrets are created at deploy time in the namespace-scoped `runtime` Secret; never add them to Terraform state, Helm values, Git, logs, or evidence. Keep kubeconfig outside tracked source (the ignored `tmp/day2` lab path is allowed). Local database/cache data is disposable.
- Day 4 scope: `observability/`, `scripts/day4`, `scripts/chaos`, Day 4 Helm observability templates and values, `ai-prompts/day4.md`, Day 4 evidence. Baseline must be at least one hour before any anomaly alert counts; a shorter window is recorded as insufficient, never lowered. Three incidents are three separate injections. RCA may cite only samples collected from Prometheus by `scripts/day4/collect-samples.py`; a value typed by hand or summarized by a model is fabrication and the verifier re-queries every one of them. Chaos flags are refused outside fixture mode. Slack webhook and Grafana admin password live only in namespace Secrets, never in Git, values, logs or evidence.
...
- An anomaly band describes deviation from the recent past, not correctness. Detection names the symptom; correlation across signals names the cause, and an RCA without a cited metric and timestamp is a hypothesis, not a finding. Drift is an ML-team concern with its own lifecycle: DevOps supplies infrastructure evidence and executes rollback, and never retrains a model.
...
- Day 2 requirements and review: specification section 6, `docs/lab-guides/Day2-MCP-Protocol.md`, `tools/mcp/day2/README.md`, `debug-session-day2.md`, `ai-prompts/day2.md`.
- Day 3 local design and operations: `infra/SPEC.md`, `infra/README.md`, `.github/workflows/iac.yml`, `ai-prompts/day3.md`.
- Day 4 requirements and operations: specification section 8, `docs/lab-guides/Day4-AIOps-Observability.md`, `observability/README.md`, `observability/RUNBOOK.md`, `observability/rules/day4-rules.yaml`, `observability/mlops-overview-notes.md`, `ai-prompts/day4.md`.
